...
const renderValidation = (els, errorCode, i18n) => {
    if (errorCode) {
        ...
        els.feedback.textContent = i18n.t(errorCode); // p.ej. "No debe estar vacío"
        els.feedback.classList.remove('text-success');
        els.feedback.classList.add('text-danger');
    } else {
        els.input.classList.remove('is-invalid');
        els.feedback.textContent = i18n.t('success.added'); // "RSS cargado con éxito"
...
const renderPosts = (els, state, i18n) => {
    ...
    posts.forEach((p) => {
        ...
        btn.dataset.id = p.id;
        btn.textContent = i18n.t('ui.preview'); // "Vista previa"
